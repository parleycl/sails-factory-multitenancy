  ...
  async create(name, ...args) {
    ...
    if (multitenant) {
      ...
      if (factoryMap.get(genArgs[0])) {
        modelName = (factoryMap.get(genArgs[0]).oParent) ?
          factoryMap.get(genArgs[0]).oParent.modelId :
          factoryMap.get(genArgs[0]).modelId;
      } else if (factoryMap.get(genArgs[1])) {
        modelName = (factoryMap.get(genArgs[1]).oParent) ?
          factoryMap.get(genArgs[1]).oParent.modelId :
